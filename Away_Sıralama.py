...
writer = pd.ExcelWriter('Automate_Away_sıralama_tablosu.xlsx', engine='xlsxwriter')
...
with tqdm(total=total_iterations, desc="Processing") as pbar:
    for lig_id in lig_ids:
        ...
        for week in weeks:
            ...
            # Select away teams and their goals
            away_teams = df_week[['Game Week', 'away_team_name', 'home_team_goal_count', 'away_team_goal_count']].copy()

            # Rename columns for consistency
            away_teams.columns = ['Game Week', 'Team Name', 'Goal Defeated', 'Goal Count']

            # Calculate the Average column
            away_teams['Average'] = away_teams['Goal Count'] - away_teams['Goal Defeated']

            # Calculate the Point column
            away_teams['Point'] = away_teams.apply(lambda row: 3 if row['Goal Count'] > row['Goal Defeated'] else (1 if row['Goal Count'] == row['Goal Defeated'] else 0), axis=1)

            # Calculate Win, Draw, Loss columns
            away_teams['Win'] = away_teams['Point'].apply(lambda x: 1 if x == 3 else 0)
            away_teams['Draw'] = away_teams['Point'].apply(lambda x: 1 if x == 1 else 0)
            away_teams['Loss'] = away_teams['Point'].apply(lambda x: 1 if x == 0 else 0)
            ...
            # Select home teams and set their statistics to zero
            home_teams = df_week[['Game Week', 'home_team_name']].copy()
            home_teams.columns = ['Game Week', 'Team Name']
            home_teams['Goal Count'] = 0
            home_teams['Goal Defeated'] = 0
            home_teams['Average'] = 0
            home_teams['Point'] = 0
            home_teams['Win'] = 0
            home_teams['Draw'] = 0
            home_teams['Loss'] = 0
            ...
            combined_teams = pd.concat([away_teams, home_teams])
            ...
            for team in weekly_summary['Team Name']:
                if team in away_teams['Team Name'].values:
            ...
            all_team_total_goal = away_teams['Goal Count'].sum()
            weekly_summary['All Team Total Goal'] = all_team_total_goal

            # Sort teams by their rank
            weekly_summary = weekly_summary.sort_values(by=['Cumulative Point', 'Cumulative Goal Count'], ascending=[False, False])
            weekly_summary['Rank'] = range(1, len(weekly_summary) + 1)

            # Calculate Rank Difference column
            if not previous_week_summaries[lig_id].empty:
                previous_week_summary = previous_week_summaries[lig_id]
                previous_week_summary = previous_week_summary[['Team Name', 'Rank']].rename(columns={'Rank': 'Previous Rank'})
                weekly_summary = weekly_summary.merge(previous_week_summary, on='Team Name', how='left')
                weekly_summary['Rank Diff'] = weekly_summary['Previous Rank'] - weekly_summary['Rank']  # Reversed
                weekly_summary.drop(columns=['Previous Rank'], inplace=True)
            else:
                weekly_summary['Rank Diff'] = 0  # Rank Diff is 0 for the first week

            # Calculate Total Rank Difference and add it to the summary
            for team in weekly_summary['Team Name']:
                total_rank_diff[lig_id][team] = total_rank_diff[lig_id].get(team, 0) + weekly_summary.loc[weekly_summary['Team Name'] == team, 'Rank Diff'].values[0]
                weekly_summary.loc[weekly_summary['Team Name'] == team, 'Total Rank Diff'] = total_rank_diff[lig_id][team]
...
print(f"Process completed, please check 'Away_haftalık_sıralama_tablosu.xlsx'. Number of sheets created: {sheet_count}")
