...
writer = pd.ExcelWriter('Modul4_Genel_sıralama_tablosu.xlsx', engine='xlsxwriter')
...
for lig_id in tqdm(lig_ids, desc='Processing Lig IDs'):
    for week in tqdm(weeks, desc=f'Processing weeks for Lig ID {lig_id}', leave=False):
        ...
        final_sheet_name = f'Week{int(week)}_F_Lig_4_{lig_id}'[:31]
...
print(f"Process completed, please check 'Modul4_Genel_sıralama_tablosu.xlsx'. Number of sheets created: {sheet_count}")
