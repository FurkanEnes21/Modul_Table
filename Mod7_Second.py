...
output_file = 'Modul7_Genel_sıralama_tablosu.xlsx'
output_df = pd.read_excel(output_file, sheet_name=None)

# Create an Excel writer
writer = pd.ExcelWriter('Final_Modul7_Genel_sıralama_tablosu.xlsx', engine='xlsxwriter')
        ...
        for index, row in weekly_summary.iterrows():
            ...
            if pd.notna(pre_home_rank) and pd.notna(pre_away_rank):  # Ensure ranks are not NaN
                if pre_home_rank < pre_away_rank:
                    ...
                    weekly_summary.at[index, 'Away Team Goal Count'] = row['Away Goals']= 0
                    weekly_summary.at[index, 'Away Team Goal Defeated'] = row['Home Goals']= 0
                    weekly_summary.at[index, 'Away Team Average'] = row['Away Team Average'] = row['Away Goals'] - row['Home Goals']
                    weekly_summary.at[index, 'Away Team Average'] = 0
                    weekly_summary.at[index, 'Away Team Point'] = row['Away Team Point']= 0
                    weekly_summary.at[index, 'Away Team Rank'] = row['Away Team Rank'] = 0
                    weekly_summary.at[index, 'Away Team Rank Diff'] = row['Away Team Rank Diff'] = 0
                    weekly_summary.at[index, 'Away Team Total Rank Diff'] = row[' Away Team Total Rank Diff'] = 0
                    weekly_summary.at[index, 'Match Played'] = 0
                else:
                    # Away team has a lower rank (higher rank number)
                    weekly_summary.at[index, 'Home Team Goal Count'] = row['Home Goals']
                    weekly_summary.at[index, 'Home Team Goal Defeated'] = row['Away Goals']
                    weekly_summary.at[index, 'Home Team Average'] = row['Home Team Average'] = row['Home Goals'] - row['Away Goals']
                    weekly_summary.at[index, 'Home Team Point'] = row['Home Team Point']
                    weekly_summary.at[index, 'Home Team Rank'] = row['Home Team Rank']
                    weekly_summary.at[index, 'Home Team Rank Diff'] = row['Home Team Rank Diff']
                    weekly_summary.at[index, 'Home Team Total Rank Diff'] = row['Home Team Total Rank Diff']
...
print("Process completed, please check 'Final_Modul7_Genel_sıralama_tablosu.xlsx'.")