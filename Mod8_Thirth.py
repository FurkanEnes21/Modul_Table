...
# df = pd.read_excel('Final_Modul8_Genel_sıralama_tablosu.xlsx', engine='openpyxl', sheet_name=None)

# # Create a new Excel writer for the cumulative calculations
# writer = pd.ExcelWriter('Modul8_Cumulative_Statistics.xlsx', engine='xlsxwriter')
...
#     sheet_df['Home Win'] = sheet_df['Home Team Point'].apply(lambda x: 1 if x == 3 else 0)
#     sheet_df['Home Draw'] = sheet_df['Home Team Point'].apply(lambda x: 1 if x == 1 else 0)
#     sheet_df['Home Loss'] = sheet_df['Home Team Point'].apply(lambda x: 1 if x == 0 else 0)

#     # Calculate Win, Draw, and Loss for Away Team
#     sheet_df['Away Win'] = sheet_df['Away Team Point'].apply(lambda x: 1 if x == 3 else 0)
#     sheet_df['Away Draw'] = sheet_df['Away Team Point'].apply(lambda x: 1 if x == 1 else 0)
#     sheet_df['Away Loss'] = sheet_df['Away Team Point'].apply(lambda x: 1 if x == 0 else 0)
...
#     home_ranking_df = sheet_df[['Home Team', 'Home Team Point']].groupby('Home Team').sum().reset_index()
...
#     away_ranking_df = sheet_df[['Away Team', 'Away Team Point']].groupby('Away Team').sum().reset_index()
...
#     sheet_df['Home Team Rank'] = sheet_df['Home Team'].map(home_ranking_df.set_index('Home Team')['Home Team Rank'])
#     sheet_df['Away Team Rank'] = sheet_df['Away Team'].map(away_ranking_df.set_index('Away Team')['Away Team Rank'])

#     # Calculate rank differences
#     sheet_df['Home Team Rank Diff'] = sheet_df['Home Team Rank'] - sheet_df['Away Team Rank']
#     sheet_df['Away Team Rank Diff'] = sheet_df['Away Team Rank'] - sheet_df['Home Team Rank']

#     # Calculate total rank differences
#     sheet_df['Home Team Total Rank Diff'] = sheet_df.groupby('Home Team')['Home Team Rank Diff'].cumsum()
#     sheet_df['Away Team Total Rank Diff'] = sheet_df.groupby('Away Team')['Away Team Rank Diff'].cumsum()

#     # Add cumulative values to the DataFrame
#     sheet_df['Cumulative Home Team Goal Count'] = sheet_df['Home Team'].map(cumulative_data[lig_id]['Cumulative Goal Count'])
#     sheet_df['Cumulative Home Team Goal Defeated'] = sheet_df['Home Team'].map(cumulative_data[lig_id]['Cumulative Goal Defeated'])
#     sheet_df['Cumulative Home Team Average'] = sheet_df['Home Team'].map(cumulative_data[lig_id]['Cumulative Average'])
#     sheet_df['Cumulative Home Team Point'] = sheet_df['Home Team'].map(cumulative_data[lig_id]['Cumulative Point'])
#     sheet_df['Cumulative Home Team Match Play'] = sheet_df['Home Team'].map(cumulative_data[lig_id]['Cumulative Match Play'])
#     sheet_df['Cumulative Home Team Win'] = sheet_df['Home Team'].map(cumulative_data[lig_id]['Cumulative Win'])
#     sheet_df['Cumulative Home Team Draw'] = sheet_df['Home Team'].map(cumulative_data[lig_id]['Cumulative Draw'])
#     sheet_df['Cumulative Home Team Loss'] = sheet_df['Home Team'].map(cumulative_data[lig_id]['Cumulative Loss'])

#     sheet_df['Cumulative Away Team Goal Count'] = sheet_df['Away Team'].map(cumulative_data[lig_id]['Cumulative Goal Count'])
#     sheet_df['Cumulative Away Team Goal Defeated'] = sheet_df['Away Team'].map(cumulative_data[lig_id]['Cumulative Goal Defeated'])
#     sheet_df['Cumulative Away Team Average'] = sheet_df['Away Team'].map(cumulative_data[lig_id]['Cumulative Average'])
#     sheet_df['Cumulative Away Team Point'] = sheet_df['Away Team'].map(cumulative_data[lig_id]['Cumulative Point'])
#     sheet_df['Cumulative Away Team Match Play'] = sheet_df['Away Team'].map(cumulative_data[lig_id]['Cumulative Match Play'])
#     sheet_df['Cumulative Away Team Win'] = sheet_df['Away Team'].map(cumulative_data[lig_id]['Cumulative Win'])
#     sheet_df['Cumulative Away Team Draw'] = sheet_df['Away Team'].map(cumulative_data[lig_id]['Cumulative Draw'])
#     sheet_df['Cumulative Away Team Loss'] = sheet_df['Away Team'].map(cumulative_data[lig_id]['Cumulative Loss'])

#     # Calculate All Team Total Goal for the week
#     all_team_total_goal = (sheet_df['Home Team Goal Count'] + sheet_df['Away Team Goal Count']).sum()
#     sheet_df['All Team Total Goal'] = all_team_total_goal

#     # Calculate Cumulative Point Ratio for home and away teams
#     sheet_df['Cumulative Home Team Point Ratio'] = sheet_df.apply(
#         lambda row: row['Cumulative Home Team Point'] / row['Cumulative Home Team Match Play'] if row['Cumulative Home Team Match Play'] != 0 else 0,
#         axis=1
#     )
#     sheet_df['Cumulative Away Team Point Ratio'] = sheet_df.apply(
#         lambda row: row['Cumulative Away Team Point'] / row['Cumulative Away Team Match Play'] if row['Cumulative Away Team Match Play'] != 0 else 0,
#         axis=1
#     )

#     # Calculate Cumulative Goal Defeated Ratio for home and away teams
#     sheet_df['Total Cumulative Home Team Goal Defeated Ratio'] = sheet_df.apply(
#         lambda row: row['Cumulative Home Team Goal Defeated'] / row['Cumulative Home Team Match Play'] if row['Cumulative Home Team Match Play'] != 0 else 0,
#         axis=1
#     )
#     sheet_df['Total Cumulative Away Team Goal Defeated Ratio'] = sheet_df.apply(
#         lambda row: row['Cumulative Away Team Goal Defeated'] / row['Cumulative Away Team Match Play'] if row['Cumulative Away Team Match Play'] != 0 else 0,
#         axis=1
#     )

#     # Calculate Cumulative Goal Count Ratio for home and away teams
#     sheet_df['Total Cumulative Home Team Goal Count Ratio'] = sheet_df.apply(
#         lambda row: row['Cumulative Home Team Goal Count'] / row['Cumulative Home Team Match Play'] if row['Cumulative Home Team Match Play'] != 0 else 0,
#         axis=1
#     )
#     sheet_df['Total Cumulative Away Team Goal Count Ratio'] = sheet_df.apply(
#         lambda row: row['Cumulative Away Team Goal Count'] / row['Cumulative Away Team Match Play'] if row['Cumulative Away Team Match Play'] != 0 else 0,
#         axis=1
#     )
...
#     column_order = [
#         'Lig ID', 'Lig Tipi', 'Game Week', 'Match Play', 'Home Team', 'Away Team', 'Home Team Goal Count', 'Away Team Goal Count', 'Home Team Goal Defeated', 'Away Team Goal Defeated',
...
#         'Cumulative Home Team Average', 'Cumulative Home Team Point', 'Cumulative Home Team Match Play', 'Cumulative Home Team Win', 'Cumulative Home Team Draw', 'Cumulative Home Team Loss',
...
#         'Cumulative Away Team Average', 'Cumulative Away Team Point', 'Cumulative Away Team Match Play', 'Cumulative Away Team Win', 'Cumulative Away Team Draw', 'Cumulative Away Team Loss',
...
# print(f"Cumulative statistics calculation completed. Please check 'Modul8_Cumulative_Statistics.xlsx'.")
...
# # Load the Excel file with the previous data
# df = pd.read_excel('Final_Modul9_Genel_sıralama_tablosu.xlsx', engine='openpyxl', sheet_name=None)

# # Create a new Excel writer for the cumulative calculations
# writer = pd.ExcelWriter('Modul9_Cumulative_Statistics.xlsx', engine='xlsxwriter')
...
#     match_play_1_df = sheet_df[sheet_df['Match Play'] == 1].copy()
#     match_play_0_df = sheet_df[sheet_df['Match Play'] == 0].copy()
...
#     for col in ['Goal Count', 'Goal Defeated', 'Average', 'Point', 'Match Play', 'Win', 'Draw', 'Loss']:
#         final_df[f'Cumulative Home Team {col}'] = final_df.apply(
#             lambda row: cumulative_data[lig_id][f'Cumulative {col}'][row['Home Team']] if row['Match Play'] == 1 else 0,
#             axis=1
#         )
#         final_df[f'Cumulative Away Team {col}'] = final_df.apply(
#             lambda row: cumulative_data[lig_id][f'Cumulative {col}'][row['Away Team']] if row['Match Play'] == 1 else 0,
#             axis=1
#         )
...
# print(f"Cumulative statistics calculation completed. Please check 'Modul9_Cumulative_Statistics.xlsx'.")
...
for sheet_name in tqdm(df.keys(), desc='Processing Sheets'):
    ...
    for index, row in match_play_1_df.iterrows():
        ...
        if home_team not in cumulative_data[lig_id]['Cumulative Goal Count']:
            ...
            cumulative_data[lig_id]['Cumulative Average'][home_team] = 0
            cumulative_data[lig_id]['Cumulative Point'][home_team] = 0
            cumulative_data[lig_id]['Cumulative Match Played'][home_team] = 0
            cumulative_data[lig_id]['Cumulative Win'][home_team] = 0
            cumulative_data[lig_id]['Cumulative Draw'][home_team] = 0
            cumulative_data[lig_id]['Cumulative Loss'][home_team] = 0
            cumulative_data[lig_id]['Rank'][home_team] = []
        ...
        if away_team not in cumulative_data[lig_id]['Cumulative Goal Count']:
            ...
            cumulative_data[lig_id]['Cumulative Average'][away_team] = 0
            cumulative_data[lig_id]['Cumulative Point'][away_team] = 0
            cumulative_data[lig_id]['Cumulative Match Played'][away_team] = 0
            cumulative_data[lig_id]['Cumulative Win'][away_team] = 0
            cumulative_data[lig_id]['Cumulative Draw'][away_team] = 0
            cumulative_data[lig_id]['Cumulative Loss'][away_team] = 0
            cumulative_data[lig_id]['Rank'][away_team] = []
        ...
        cumulative_data[lig_id]['Cumulative Average'][home_team] += row['Home Team Average']
        cumulative_data[lig_id]['Cumulative Point'][home_team] += row['Home Team Point']
        cumulative_data[lig_id]['Cumulative Match Played'][home_team] += 1
        cumulative_data[lig_id]['Cumulative Win'][home_team] += row['Home Win']
        cumulative_data[lig_id]['Cumulative Draw'][home_team] += row['Home Draw']
        cumulative_data[lig_id]['Cumulative Loss'][home_team] += row['Home Loss']
        ...
        cumulative_data[lig_id]['Cumulative Average'][away_team] += row['Away Team Average']
        cumulative_data[lig_id]['Cumulative Point'][away_team] += row['Away Team Point']
        cumulative_data[lig_id]['Cumulative Match Played'][away_team] += 1
        cumulative_data[lig_id]['Cumulative Win'][away_team] += row['Away Win']
        cumulative_data[lig_id]['Cumulative Draw'][away_team] += row['Away Draw']
        cumulative_data[lig_id]['Cumulative Loss'][away_team] += row['Away Loss']
    ...
    # Calculate Cumulative Goal Count Ratio for home and away teams
    final_df['Total Cumulative Home Team Goal Count Ratio'] = final_df.apply(
        lambda row: row['Cumulative Home Team Goal Count'] / row['Cumulative Home Team Match Played'] if row['Cumulative Home Team Match Played'] != 0 else 0,
        axis=1
    )
    final_df['Total Cumulative Away Team Goal Count Ratio'] = final_df.apply(
